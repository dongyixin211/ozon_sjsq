...
const outputDir = path.resolve(args.outputDir || path.join(repoRoot, ".codex-work", "zhuobu-scene1-reference-mask-batch"));
...
const sceneIndex = Number(args.scene || 1);
...
const { renderMockupsWithTemplate, invalidateMockupTemplateCache } = await import(`${rendererPath}?scene1ReferenceMaskBatch=${Date.now()}`);
...
for (const [name, buffer] of Object.entries(masks)) {
  await fs.writeFile(path.join(outputDir, `scene-${sceneId}-${name}.png`), buffer);
}
...
const candidates = [
  ...
  ["mask-majority-linear-0p28", (template) => {
    applyMaskCandidate(template, "majority");
    scene(template).linearLightStrength = 0.28;
  }],
  ["mask-majority-linear-0p44", (template) => {
    applyMaskCandidate(template, "majority");
    scene(template).linearLightStrength = 0.44;
  }],
];
...
for (const [name, mutate] of candidates) {
  ...
  for (const [maskName, buffer] of Object.entries(masks)) {
    await fs.writeFile(path.join(targetDir, "masks", `scene-${sceneId}-${maskName}.png`), buffer);
  }
  ...
  for (const sample of samples) {
    ...
    if (args.keepImages === "true") {
      await fs.writeFile(path.join(outputDir, `${name}-${sample.sku}-cloud-${sceneId}.png`), renderedScene.buffer);
    }
...
async function buildReferenceMasks(sharp, samples, threshold) {
  ...
  if (!baseLayer?.file) throw new Error(`scene ${sceneIndex} base layer not found`);
...
function applyMaskCandidate(template, maskName) {
  for (const layer of scene(template).layers) {
    ...
    layer.mask = `masks/scene-${sceneId}-${maskName}.png`;
    layer.maskLeft = 0;
    layer.maskTop = 0;
    layer.maskWidth = 800;
