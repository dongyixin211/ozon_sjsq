...
const outputDir = path.resolve(args.outputDir || path.join(repoRoot, ".codex-work", "zhuobu-scene6-restore-structure"));
...
const { renderMockupsWithTemplate, invalidateMockupTemplateCache } = await import(`${rendererPath}?scene6RestoreStructure=${Date.now()}`);
...
const candidates = [
  ["baseline", () => {}],
  ["restore-mesh", restoreScene6Meshes],
  ["restore-normal-layers", restoreScene6NormalLayers],
  ["restore-mesh-and-normal-layers", (template) => {
    restoreScene6Meshes(template);
    restoreScene6NormalLayers(template);
  }],
  ["restore-normal-011", (template) => restoreScene6NormalLayers(template, new Set(["图层 10"]))],
  ["restore-normal-005", (template) => restoreScene6NormalLayers(template, new Set(["图层 8"]))],
  ["restore-normal-002", (template) => restoreScene6NormalLayers(template, new Set(["图层 6"]))],
  ["restore-mesh-normal-011", (template) => {
    restoreScene6Meshes(template);
    restoreScene6NormalLayers(template, new Set(["图层 10"]));
  }],
  ["restore-mesh-normal-005", (template) => {
    restoreScene6Meshes(template);
    restoreScene6NormalLayers(template, new Set(["图层 8"]));
  }],
  ["restore-mesh-normal-002", (template) => {
    restoreScene6Meshes(template);
    restoreScene6NormalLayers(template, new Set(["图层 6"]));
  }],
];
...
console.log(path.join(outputDir, "scene-06-restore-structure.jpg"));
...
function restoreScene6Meshes(template) {
  const oldScene = scene6(oldTemplate);
...
function restoreScene6NormalLayers(template, names) {
  const scene = scene6(template);
  const existingNames = new Set(scene.layers.map((layer) => layer.name));
  const oldScene = scene6(oldTemplate);
  for (const oldLayer of oldScene.layers) {
    if (oldLayer.kind !== "image" || oldLayer.blendMode !== "normal" || oldLayer.order === 0) {
      continue;
    }
    if (names && !names.has(oldLayer.name)) {
      continue;
    }
    if (existingNames.has(oldLayer.name)) {
      continue;
    }
    scene.layers.push(structuredClone(oldLayer));
  }
}
...
async function calculateCoverage(sharp, referenceBuffer, cloudBuffer) {
  const templateBaseLayer = scene6(baseTemplate).layers.find((layer) => (
    layer.kind === "image" && layer.blendMode === "normal" && layer.file === "layers/scene-06-layer-014.png"
  ));
  if (!templateBaseLayer?.file) {
    throw new Error("scene 6 base layer not found");
  }
  const [reference, cloud, base] = await Promise.all([
    rawRgb(sharp, referenceBuffer),
    rawRgb(sharp, cloudBuffer),
    rawRgb(sharp, await fs.readFile(path.join(baseDir, templateBaseLayer.file))),
  ]);
  let psPattern = 0;
  let cloudPattern = 0;
  let missed = 0;
...
async function createComparison(sharp, summaries, referenceBuffer, outputDir) {
  ...
  await sharp({
    ...
    .jpeg({ quality: 90 })
    .toFile(path.join(outputDir, "scene-06-restore-structure.jpg"));
}
