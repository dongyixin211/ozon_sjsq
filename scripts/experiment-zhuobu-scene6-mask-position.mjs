...
const outputDir = path.resolve(args.outputDir || path.join(repoRoot, ".codex-work", "zhuobu-scene6-mask-position"));
...
const { renderMockupsWithTemplate, invalidateMockupTemplateCache } = await import(`${rendererPath}?scene6MaskPosition=${Date.now()}`);
...
const candidates = [
  ["baseline", async () => {}],
  ["scene6-red-alpha", async (_template, targetDir) => convertScene6Masks(targetDir, "red-alpha")],
  ["scene6-alpha", async (_template, targetDir) => convertScene6Masks(targetDir, "alpha")],
  ["scene6-alpha-positioned", async (template, targetDir) => {
    await convertScene6Masks(targetDir, "alpha");
    setScene6MasksPositioned(template);
  }],
  ["scene6-red-alpha-positioned", async (template, targetDir) => {
    await convertScene6Masks(targetDir, "red-alpha");
    setScene6MasksPositioned(template);
  }],
  ["scene6-no-mask", async (template) => removeScene6Masks(template)],
];
...
for (const [name, mutate] of candidates) {
  ...
  console.log(JSON.stringify({ name, ...metric }));
}
...
console.log(path.join(outputDir, "scene-06-mask-position.jpg"));
...
function removeScene6Masks(template) {
  for (const layer of scene6(template).layers) {
...
async function convertScene6Masks(targetDir, mode) {
  const masks = [
...
async function createComparison(sharp, summaries, referenceBuffer, outputDir) {
  ...
  await sharp({
    ...
    .jpeg({ quality: 90 })
    .toFile(path.join(outputDir, "scene-06-mask-position.jpg"));
}
