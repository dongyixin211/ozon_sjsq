...
const outputDir = path.resolve(args.outputDir || path.join(repoRoot, ".codex-work", "zhuobu-scene6-reference-mask-batch"));
...
const { renderMockupsWithTemplate, invalidateMockupTemplateCache } = await import(`${rendererPath}?scene6ReferenceMaskBatch=${Date.now()}`);
...
for (const [name, buffer] of Object.entries(masks)) {
  await fs.writeFile(path.join(outputDir, `scene-06-${name}.png`), buffer);
}

const candidates = [
  ["baseline", () => {}],
  ["mask-union-mesh-no-duplicates", (template) => applyMaskCandidate(template, "union", { mesh: true, noDuplicates: true })],
  ["mask-majority-mesh-no-duplicates", (template) => applyMaskCandidate(template, "majority", { mesh: true, noDuplicates: true })],
  ["mask-intersection-mesh-no-duplicates", (template) => applyMaskCandidate(template, "intersection", { mesh: true, noDuplicates: true })],
  ["mask-soft-mesh-no-duplicates", (template) => applyMaskCandidate(template, "soft", { mesh: true, noDuplicates: true })],
...
for (const [name, mutate] of candidates) {
  ...
  for (const [maskName, buffer] of Object.entries(masks)) {
    await fs.writeFile(path.join(targetDir, "masks", `scene-06-${maskName}.png`), buffer);
  }
  ...
  for (const sample of samples) {
    ...
    const rendered = await renderMockupsWithTemplate({ templateDir: name, sourceBuffer, sku: sample.sku, sceneIndexes: [6] });
    const scene = rendered.scenes[0];
    if (args.keepImages === "true") {
      await fs.writeFile(path.join(outputDir, `${name}-${sample.sku}-cloud-06.png`), scene.buffer);
    }
    const referenceBuffer = await fs.readFile(sample.refs[6]);
...
async function buildReferenceMasks(sharp, samples, threshold) {
  ...
  for (const sample of samples) {
    const reference = await rawRgb(sharp, await fs.readFile(sample.refs[6]));
...
function applyMaskCandidate(template, maskName, options = {}) {
  for (const layer of scene6(template).layers) {
    if (layer.kind !== "replace") continue;
    layer.mask = `masks/scene-06-${maskName}.png`;
    layer.maskLeft = 0;
    layer.maskTop = 0;
    layer.maskWidth = 800;
