...
const kernels = options.kernels.length ? options.kernels : ["template", "nearest", "cubic", "mitchell", "lanczos2", "lanczos3"];

const { default: sharp } = await import(sharpPath);
const { renderFangjinMockups } = await import(`${rendererPath}?replacementKernel=${Date.now()}`);
  ...
  for (const kernel of kernels) {
    ...
    for (const sample of sampleInputs) {
      const rendered = await renderFangjinMockups({
        ...
        replacementResizeKernel: kernel === "template" ? undefined : kernel,
      });
    ...
    const best = rows[0];
    console.log(`场景 ${sceneIndex} kernel=${kernel} directMae=${rows.find((row) => row.kernel === kernel).directMae}; 当前最佳 ${best.kernel} ${best.directMae}`);
  }
  rows.sort((left, right) => left.directMae - right.directMae);
...
function parseArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--kernels") {
      parsed.kernels = (args[index + 1] || "").split(",").map((item) => item.trim()).filter(Boolean);
      index += 1;
      continue;
    }
    if (arg === "--sample-limit") {
