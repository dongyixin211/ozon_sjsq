param(
  ...
  [string]$OutputRoot = "E:\tool\ozon_sjsq\dist\mockup-inspect"
)
...
$jsxPath = "E:\tool\ozon_sjsq\scripts\inspect-one-psd-mockup.jsx"
$photoshop = New-Object -ComObject Photoshop.Application
$photoshop.Visible = $false
$photoshop.DoJavaScriptFile($jsxPath)

Get-Content -Raw -Encoding UTF8 -LiteralPath (Join-Path $outputDir "report.json")
