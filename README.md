...
- 上架后自动处理：商品上架成功后自动生成条码、补库存并加入指定促销活动。
- 商品分析：按详情页浏览量查看商品，并将勾选商品按同类目、同类型每 20 个合并商品卡。
...
7. 如启用“上架后自动处理”，等待商品 ID 后依次生成条码、补库存和加入促销；单项失败会写入任务日志和结果表。

浏览量与合并流程：

1. 选择统计日期，按商品详情页浏览量降序查询商品。
2. 可设置最低浏览量，只勾选需要合并的高浏览商品。
3. 程序按“类目 + 类型”分组，每组最多 20 个商品。
4. 仅更新型号/合并属性，不改标题、图片、价格和库存；不足 2 个或缺少型号属性的商品会跳过。
